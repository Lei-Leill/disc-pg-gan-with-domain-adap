...
def disc_along_genome(iterator, input_folder, output_file):
    ...
    for start_idx in range(0, final_end, NUM_SNPS):
        ...
        if region is not None:
            #corrected = np.zeros((1, iterator.num_samples, NUM_SNPS, 2), dtype=np.float32)
            corrected = np.zeros((1, 198, NUM_SNPS, 2), dtype=np.float32)
            try:
                corrected[0] = region
            except:
                corrected[0] = size_match(region, 198)
...
def size_match(region, goal_size):
    for i in range (16):
        ran_index = random.randint(0, 181)
        sample = np.expand_dims(region[ran_index], axis=0)
        region = np.concatenate((region, sample), axis = 0)
    return region
